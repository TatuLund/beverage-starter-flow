...
class ClientCategories extends PolymerElement {
  static get template() {
    return html`
    <style>
      vaadin-grid {
...
`;
  ...
  update() {
    ...
    connectServices.categories(search.value).then(items => this.$.grid.items = items);
  }
}

window.customElements.define(ClientCategories.is, ClientCategories);
